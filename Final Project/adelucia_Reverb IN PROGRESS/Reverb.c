...
//Compile with:
//clang Reverb.c -o Reverb -lsndfile
//Run with:
//./Reverb

#define kInputFileName "CheeseSynth.wav"
#define kOutputFileName "AddedReverb.wav"
...
//Function prototypes
int openInputSndFile(SoundFile *inFile, char *fileName);
...
int main(int argc, char *argv[]){
        if (argc != 2) {
                printf("If you're seeing this, please enter \"%s FileName.wav\"\n", argv[0]);
                return 1;
        }
...
void process(float *inBuffer, float *outBuffer, sf_count_t bufferSize){
        ...
        //         }
        // }
//LPF (inBuffer, outBuffer, bufferSize, 55);
Reverb (inBuffer, outBuffer, bufferSize, 0.5 ,10, 4000, 0.5);
}

int openInputSndFile(SoundFile *sndFile, char *fileName){
